...
@section('title')
    @lang('messages.edit') @lang('dashboard.branches')
@endsection
...
@section('content')
    <section class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1> @lang('messages.edit') @lang('dashboard.branches') </h1>
                </div>
                <div class="col-sm-6">
                ...
                <!-- general form elements -->
                    <div class="card card-primary">
                        <div class="card-header">
                            <h3 class="card-title">@lang('messages.edit') @lang('dashboard.reservation_branches') </h3>
                        </div>
                        <!-- /.card-header -->
                        <!-- form start -->
                        <form role="form" id="post-form" action="{{route('restaurant.reservation.branch.update' , $branch->id)}}" method="post"
                            ...
                            <div class="card-body">
                                ...
                                <div class="form-group">
                                    <label class="control-label"> @lang('dashboard.entry.location_link') </label>
                                    <input type="url" name="location_link" class="form-control" value="{{$branch->location_link}}">
                                    @if ($errors->has('location_link'))
                                        <span class="help-block">
                                            <strong style="color: red;">{{ $errors->first('location_link') }}</strong>
                                        </span>
                                    @endif
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="status" id="" class="select2 form-control ">
                                        <option value="1" {{$branch->status == true ? 'selected' : ''}}>{{ trans('dashboard.yes') }}</option>
                                        <option value="0" {{$branch->status == false ? 'selected' : ''}}>{{ trans('dashboard.no') }}</option>
                                    </select>
