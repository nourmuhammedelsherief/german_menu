...
@section('title')
    @lang('messages.edit') @lang('messages.menu_periods')
@endsection
@section('style')
@endsection
@section('content')
    <section class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1> @lang('messages.edit') @lang('messages.menu_periods') </h1>
                </div>
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        ...
                        <li class="breadcrumb-item active">
                            <a href="{{route('menu_foodics_days.index' , $branch->id)}}">
                                @lang('messages.menu_periods')
                            </a>
                        </li>
                    </ol>
                </div>
                ...
                <!-- general form elements -->
                    <div class="card card-primary">
                        <div class="card-header">
                            <h3 class="card-title">@lang('messages.edit') @lang('messages.menu_periods') </h3>
                        </div>
                        <!-- /.card-header -->
                        <!-- form start -->
                        <form role="form" action="{{route('menu_foodics_days.update' , $period->id)}}" method="post"
                                ...
                                <div class="form-group">
                                    ...
                                    @foreach($days as $day)
                                        <input type="checkbox" name="day_id[]" value="{{$day->id}}"
                                            {{\App\Models\RestaurantOrderPeriodDay::wherePeriodId($period->id)->where('day_id' , $day->id)->first() != null ? 'checked' : ''}}
                                        >
