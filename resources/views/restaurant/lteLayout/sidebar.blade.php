                    ...
                    @if ($user->type == 'restaurant' or $integration_permission and $user->type == 'employee')
                        ...
                        @php
                            $waiterSubscription = \App\Models\ServiceSubscription::whereRestaurantId($user->type == 'employee' ? $user->restaurant_id : $user->id)
                                ...
                                ->first();
                        @endphp
                        @if (isset($waiterSubscription->id))
                            <li
                        ...
                        @if ($checkReservation)
                            <li
                                ...
                                <ul class="nav nav-treeview">
                                    ...
                                    {{-- bank --}}
                                    {{-- @if (\App\Models\ServiceSubscription::whereRestaurantId($user->id)->whereHas('service', function ($query) {
                        ...
                        @if ($check_branch)
                            ...
                            <li class="nav-item">
                                <a href="{{ route('FoodicsOrderTable', $check_branch->id) }}"
                                    ...
                                    <span class="badge badge-info right">
                                        {{ \App\Models\Table::whereRestaurantId($user->type == 'employee' ? $user->restaurant_id : $user->id)->whereBranchId($check_branch->id)->where('foodics_id', '!=', null)->count() }}
                                    </span>
                                    <p>
