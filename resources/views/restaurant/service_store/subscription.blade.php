    ...
    <section class="content">
        <div class="container-fluid">
            <div class="row">
                <!-- left column -->
                <div class="col-md-8">
                    @if ($is_new == 'true' and $canTentative)
                        <p style="color: red;">
                            @php
                        ...
                        <form role="form" action="{{ route('restaurant.services_store.subscription', $service->id) }}"
                            ...
                            <div class="card-body">
                                @if ($service->id == 11 and !isset($serviceSubscription->id))
                                    <div class="text-muted text-center">للإشتراك بخدمة نقاط الولاء لابد أن تكون مشترك بأحد
                                        انواع الكاشيرات الداعمة للخدمة ( كاشير ايزي منيو . كاشير فودكس )</div>
                                @endif
                                @if ($service->id != 1)
                                    @if (isset($serviceSubscription->id))
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select name="branch_id" class="form-control" onchange="showDiv(this)" required
                                                readonly>
                                                <option value="{{ $serviceSubscription->branch_id }}">
                                                    {{ $serviceSubscription->branch->name }}</option>
                                            </select>
                                        </div>
                                    @else
                                        <div class="form-group">
                                                ...
                                                @foreach ($branches as $branch)
                                                    @php
                                                        if ($service->id == 11) {
                                                            ...
                                                            $check_branch = \App\Models\ServiceSubscription::whereRestaurantId($restaurant->id)
                                                                ->whereBranchId($branch->id)
                                                                ->whereNotIn('service_id', [$service->id])
                                                                ->whereIn('status', ['active', 'tentative'])
                                                                ->first();
                                                        } elseif ($service->id == 13) {
                                                            $check_branch = \App\Models\ServiceSubscription::whereRestaurantId($restaurant->id)
                                                                ->whereBranchId($branch->id)
                                                                ->whereIn('service_id', [$service->id])
                                                                ->whereIn('status', ['active', 'tentative'])
                                                                ->first();
                                                    ...
                                                    @endphp
                                                    {{-- check if the user subscription on this service status tentative --}}
                                                    @if (
                                                        !in_array($service->id, [11]) and
                                                            ($check_branch and $check_branch->end_at <= now()->addDays(30) and $check_branch->status != 'tentative' or
                                                                $check_branch == null))
                                                    ...
                                                    @elseif($service->id == 12 and !isset($check_branch->id))
                                                        <option value="{{ $branch->id }}">
                                                            {{ app()->getLocale() == 'ar' ? $branch->name_ar : $branch->name_en }}
                                                        </option>
                                                    @elseif($service->id == 13 and !isset($check_branch->id))
                                                        <option value="{{ $branch->id }}">
                                                            {{ app()->getLocale() == 'ar' ? $branch->name_ar : $branch->name_en }}
                                                        </option>
                                                    @endif
