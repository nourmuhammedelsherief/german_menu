...
<div class="x-card card mr-0 ml-0 rounded-l" style="">
	<div class="container">
		...
		<form id="reservationForm" action="{{route('reservation.page4' ,[ $branch->id , $reservation->id])}}" method="POST" enctype="multipart/form-data">
			@csrf
			@if($type == 'bank')
		  ...
		  <div class="footer-button text-center">
			...
			<a href="{{route('reservation.page2' , [$branch->id ,$reservation->id])}}" class="btn btn-secondary " >{{ trans('messages.return') }}</a>
		  </div>
