        ...
        <div class="content mt-0 pt-1">
            ...
            @endphp
                <div class="row mb-0 mr-3 ml-3 py-3" style="border: 2px dashed #f7b538; background: #f5f5f5;">
                    ...
                    <div class="col-8 mb-1 text-left">
                        <p class="font-14 font-600  color-theme">
                            @lang('messages.' . $details->order_type)
                        </p>
                    </div>
                    ...
                    @if($branch->foodics_status == 'true')
                        <div class="col-8 mb-1 text-left">
                            <p class="font-14 font-600  color-theme">
                                {{app()->getLocale() == 'ar' ? $foodicsBranch->name_ar: $foodicsBranch->name_en}}
                            </p>
                        </div>
                    @else
                        <div class="col-8 mb-1 text-left">
                    ...
                    <div class="col-8 mb-1 text-left">
                        <p class="font-14 font-600  color-theme">
                            @if($details->payment_type == 'cash')
                                {{app()->getLocale() == 'ar' ? 'كاش' : 'Cash'}}
                            @elseif($details->payment_type == 'online')
                                {{app()->getLocale() == 'ar' ? 'أونلاين' : 'Online'}}
                                @else {{$details->payment_type}}
                            @endif
                        </p>
                    </div>
                    @if($details->payment_type == 'online' and $details->invoice_id != null)
                        <div class="col-4 mb-1">
                            <p class="font-15 font-700 color-theme">
                                {{app()->getLocale() == 'ar' ? 'رقم الدفع' : 'Invoice Id'}}
                            </p>
                        </div>
                        <div class="col-8 mb-1 text-left">
                            <p class="font-14 font-600  color-theme">
                    ...
                    <div class="col-8 mb-1 text-left">
                        <p class="font-14 font-600  color-theme">
                            {{trans('dashboard._foodics_status.' . $order->foodics_status)}}
                        </p>
                    </div>
                    @if(!empty($order->foodics_id))
                    <div class="col-8 mb-1">
                        <p class="font-15 font-700 color-theme">
                            @lang('messages.foodics_id')
                        </p>
                    </div>
                ...
                <div class="row mb-0 pt-3 mt-5 mr-1 ml-1 py-3" style="border: 1px dashed #f7b538;">
                    ...
                    @foreach($items as $item)
                        @php
                            ...
                            $taxPrice += $item->tax ;
                            $discountPrice += $item->discount_value;
                            $subTotalPrice += $item->product_price * $item->product_count;
                        @endphp
                        <div class="col-12 mb-1 mt-2">
                        ...
                        @if($item->silver_order_options->count() > 0)
                            <div class="col-12 mb-1">
                                <p class="font-15 font-600 color-theme">@lang('messages.additions') </p>
                            </div>
                            @foreach($item->silver_order_options as $option)
                                <div class="col-12 mb-1">
                                    <label for="size3-fac-radio" class="color-dark1-dark font-13">
                                        ...
                                        x {{$option->quantity}}
                                        @php
                                            $subTotalPrice += $option->price * $option->quantity;
                                        @endphp
                                    </label>
                                    <h3 class="font-11 float-left mr-2 mt-2">
                                        {{$option->price}}
                    ...
                    <div class="col-8 text-left">
                        <p class="font-14 font-600 color-theme">
                            @if($order->delivery_value != null)
                                {{$totalPrice + $order->delivery_value}}
                                {{app()->getLocale() == 'ar' ? $branch->country->currency_ar : $branch->country->currency_en}}
                            @else
                                @if($order->discount_id)
                                    {{$totalPrice - $order->discount_value}}
                                @else
                                    {{$totalPrice}}
                                @endif
                                {{app()->getLocale() == 'ar' ? $branch->country->currency_ar : $branch->country->currency_en}}
