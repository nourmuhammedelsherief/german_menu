<div class="product-details">
    ...
    <div class="card  shape-rounded rounded-lp"
        ...
        <div class="prodsld" style="position: relative">
            <a href="{{(isset($isFullPageProduct) and $isFullPageProduct ) ? session('product_back_to') : '#'}}" class="chip chip-small bg-black2 {{(isset($isFullPageProduct) and $isFullPageProduct ) ? '' : 'close-menu'}} clowind"
            ...
            <div>
                <div class="card shadow-xl">
                    @if($meal->video_type == 'youtube' and !empty($meal->video_id))
                        <div id="product-video-{{$meal->id}}"></div>
                        {{-- <iframe style="width:100%;min-height:300px" class="product-video" id="product-video-{{$meal->id}}" data-video_id="{{$meal->id}}"  src="{{$meal->video_link}}?enablejsapi=1&origin=https://easymenu.site"  frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture;enablejsapi=1;" allowfullscreen></iframe> --}}

                    @elseif($meal->video_type == 'local_video' and !empty($meal->video_id))
                        <video style="max-height: 600px;" src="{{asset($meal->video_id)}}" id="local-video-{{$meal->id}}" controls autoplay>
                            <source src="{{asset($meal->video_id)}}" type="video/mp4">
                        </video>
                    @else
    ...
    <div>
        ...
        <div class="rounded-lp mt-n2 pl-3  pr-3"
            ...
            <div class="content pt-4">
                ...
                <p class=" font-13 color-dark1-dark "
                ...
                <form method="post" action="{{route('silverAddToTableCart')}}" id="silverCartForm-{{$meal->id}}"
                    ...
                    @else
                        <form method="post" action="{{route('silverAddToCart')}}" id="silverCartForm-{{$meal->id}}"
                                                            ...
                                                            <div class="input-style input-style-2 mt-0 d-flex">
                                                                ...
                                                                @if(isset($meal->branch->id) and $meal->branch->cart == 'true')
                                                                    ...
                                                                    <button type="button"
                                                                            ...
                                                                            data-min="{{$option->min}}"
                                                                            data-max="{{$option->max}}"
                                                                            class="incress minn option_decrease"
                                                                            data="{{$option->option->id}}">-
                                                                    </button>
                            ...
                            <div class="content">
                                {{-- @if($meal->branch->cart == 'true' and $meal->branch->foodics_status == 'false')
                                    <h4 class="mb-1 font-14"
    ...
    function calcPrice(id) {
        ...
        $.each(optionPrices, function (k, v) {
            ...
            if (item.prop('checked') == true) {
                ...
                if (quantity.length > 0) var qty = parseInt(quantity.prop('value')) ;
                else var qty = 1;
    ...
    $(document).ready(function () {
        ...
        $('.activity_price').click(function () {
            ...
            if(tag.data('choose_one') == true){
                var items = $(this).parent().parent().parent().parent().find('.activity_price');
...
<script>
    ...
    $(function () {
        ...
        function onYouTubeIframeAPIReady{{$meal->id}}() {
            var temp = setInterval(function(){
                ...
                window.YT.ready(function() {
                    if(!checkYt){
                        ...
                        player_{{$meal->id}} = new YT.Player('product-video-{{$meal->id}}', {
                            height: '300',
                            width: '100%',
                            videoId: '{{$meal->video_id}}',
                            playerVars: {
                                'playsinline': 1 ,
                                'fs' :  1 ,
                                'disablekb' : 1 ,
        ...
        onYouTubeIframeAPIReady{{$meal->id}}();
        @endif
        $('.menu.menu-box-bottom .close-menu , .menu-hider').on('click', function () {
            ...
            $('#cart-count').css('z-index' , 9 );

            if(player_{{$meal->id}}){
                player_{{$meal->id}}.pauseVideo();
            }
            var videoTag = document.getElementById('local-video-{{$meal->id}}');
