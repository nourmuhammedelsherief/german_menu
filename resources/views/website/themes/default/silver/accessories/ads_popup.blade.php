...
@php
    ...
    if(isset($menu_category->id) and isset($restaurant->id) and Request::segment(3) != null): // category
        $ads = \App\Models\RestaurantAds::where('to' , 'website')->where('restaurant_id' , $restaurant->id)->where('type' , 'menu_category')->where('category_id' , $menu_category->id)->where('start_date' , '<=' , $currentDate)->where('end_date' , '>' , $currentDate)->orderBy('created_at' , 'desc')->get();
    
    elseif(isset($restaurant->id) and isset($isContactUs)):   
        $ads = \App\Models\RestaurantAds::where('to' , 'website')->where('restaurant_id' , $restaurant->id)->where('type' , 'contact_us')->where('start_date' , '<=' , $currentDate)->where('end_date' , '>' , $currentDate)->orderBy('created_at' , 'desc')->get();
    elseif(isset($restaurant->id)):
        if(isset($menu_category->id) and !$ads = \App\Models\RestaurantAds::where('to' , 'website')->where('restaurant_id' , $restaurant->id)->where('type' , 'menu_category')->where('category_id' , $menu_category->id)->where('start_date' , '<=' , $currentDate)->where('end_date' , '>' , $currentDate)->orderBy('created_at' , 'desc')->get()):

            $ads = \App\Models\RestaurantAds::where('to' , 'website')->where('restaurant_id' , $restaurant->id)->where('type' , 'main')->where('start_date' , '<=' , $currentDate)->where('end_date' , '>' , $currentDate)->orderBy('created_at' , 'desc')->get();
        else:
            $ads = \App\Models\RestaurantAds::where('to' , 'website')->where('restaurant_id' , $restaurant->id)->where('type' , 'main')->where('start_date' , '<=' , $currentDate)->where('end_date' , '>' , $currentDate)->orderBy('created_at' , 'desc')->get();
...
@if(isset($ads->id) and $ads->isAllow())
    <div id="menu-ad"
        ...
        <div class="content mb-0">
            ...
            @if($ads->content_type == 'image' or $ads->content_type == 'gif')
                <div class="image-preview close-menu" >
                    <img src="{{asset($ads->image_path)}}" style="width: 100%;" alt="">
                </div>
            @elseif($ads->content_type == 'local_video' and !empty($ads->content))
                <video src="{{asset($ads->content)}}" controls style="width:100%;max-height:400px;">
                    <source src="{{asset($ads->content)}}" type="video/mp4">
                </video>
...
<script>
	$(function(){
		// not ads 
        @if($ads->content_type != 'local_video')
        setTimeout(() => {
