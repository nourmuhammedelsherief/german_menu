@php
    $dd = 6;
    $loyaltySubscription = \App\Models\ServiceSubscription::whereRestaurantId($restaurant->id)
        ...
        <div class="sidebar">
            ...
            <div class="sidebar-body">
                @if ($table == null and $checkLoyaltyPoint == true)
                    <a href="{{ route('loyalty_points', [$restaurant->id, $branch->id]) }}">{{ trans('dashboard.loyalty_points') }}
                    </a>
                @endif
                @php
                ...
                @if (auth('web')->check() and $table == null)
                    @php
                        $restaurantId = isset($restaurant->id) ? $restaurant->id : 0;
                        $branchId = isset($branch->id) ? $branch->id : 0;
                        $check_branch = \App\Models\Branch::with('service_subscriptions')
                            ...
                            ->where('status', 'active')
                            ->where('id', $branchId)
                            ->orWhereHas('service_subscriptions', function ($q) use ($restaurantId) {
                    ...
                    @endphp
                    @if ($check_branch)
                        <a href="{{route('foodicsMyOrderDetails' , $branchId)}}">طلباتي</a>
                        <a href="{{route('foodicsLastOrderDetails' , $branchId)}}">تتبع اخر طلب</a>
                    @endif
                    <a href="{{ url('user/logout') }}">{{ trans('messages.logout') }}</a>
                @endif
            </div>
        </div>
        <a href="https://easymenu.site/" class="tail" target="_blank">
