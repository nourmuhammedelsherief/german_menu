...
<div class="x-card card mr-0 ml-0 rounded-l" style="">
    <div class="container">
        ...
        <div id="summery">
            <div class="text-center" style="font-size: 1.1rem !important;margin-top: 15px;font-weight:bold;">{{ trans('messages.reservations') }} - {{$restaurant->name}}</div>
            <div class="icons">
            ...
            <div class="details" >
                <div class="card-body" id="barcode-svg">
                    ...
                    {!! QrCode::size(130)->generate(route('reservation.summery' , [$branch->id , $reservation->id])) !!}
                 

                </div>
                <div class="text-center dd">
                    <a  href="javascript:;" id="share-reservation" style="font-size: 11px;margin-bottom: 15px;"  class="btn btn-info"><i class="fas fa-share-square"></i> {{ trans('messages.share_reservation') }}</a>
                    @if(!empty($branch->location_link)  )
                        <a  href="{{$branch->location_link}}" target="__blank" id="go-location" style="font-size: 11px;margin-bottom: 15px;"  class="btn btn-info"><i class="fas fa-map-marked-alt"></i> {{ trans('messages.go_location') }}</a>
                    @endif
                </div>
                <div class="text-center" style="font-size: 18px;
                font-weight: bold;">{{ trans('messages.order_number') }}
                    <span>#{{$reservation->id}}</span></div>
                ...
                <div class="data" style="clear: both">
                    ...
                    <p>{{ trans('messages._reservation_type.' . $reservation->type) }}
                ...
                <div class="data" style="position: relative;">
                    ...
                    <p style="">{{round($reservation->total_price)}}  {{$country->name}} <br></p>
        ...
        <div class="footer-button text-center">
            ...
            <a href="{{route('reservation.page1' , $restaurant->id)  . '?branch_id=' . (isset($reservation->table->id) ? $reservation->table->reservation_branch_id : '')}}" class="btn btn-success">{{ trans('messages.back_to_reservation') }}</a>
    ...
    <script>
        $(function(){
            ...
            $('#share-reservation').on('click' , function(){
                if (navigator.share) {
					navigator.share({
						title: "{{trans('messages.reservations')}}",
						// text : "{{trans('messages.reservations')}}" , 
                        url : "{{route('reservation.summery' , [$branch->id , $reservation->id])}}"
					})
