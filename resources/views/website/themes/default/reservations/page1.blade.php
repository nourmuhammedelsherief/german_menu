...
<div class="x-card card mr-0 ml-0 rounded-l" style="">
    <div class="container">
        ...
        <h1 class="text-center" style="font-size: 1.1rem !important;margin-top: 15px;">
            {{ trans('messages.reservations') }} - {{ $restaurant->name }}</h1>
        @if (!empty($restaurant->reservation_title))
            <p class="party-description"
                style="    margin: 10px 0 20px !important;
                text-align: center;
                font-size: 0.8rem;
                font-weight: bold;">
                {{ $restaurant->reservation_title }}
            </p>
        @endif
        ...
        @elseif(auth('web')->check())
                ...
                <div class="branches">
                    <div class="row">
                        <div class="col-12">
                            ...
                            <select name="branch_id" id="branch_id" class="form-control"  {{$branches->count() == 1 ? 'disabled' : ''}}>
            ...
            @if (isset($branch) and isset($canReservation) and $canReservation)
                ...
                <div class="footer-button text-center">
                    ...
                    @if ($restaurant->reservation_to_restaurant == 'true')
                        <a href="{{ route('sliverHome', $restaurant->name_barcode) }}"
                            class="btn btn-secondary ">{{ trans('messages.to_to_restaurant') }}</a>
                    @endif
                </div>
    ...
    <script>
        ...
        $(function() {
            $('#branch_id').on('change', function() {
                var tag = $(this);
                window.location.replace("{{ route('reservation.page1', $restaurant->id) }}?branch_id=" +
                    tag.val());
            });
            @if (isset($dates))
            ...
            $('#tables').on('click', '.time-card.package', function() {
                var tag = $(this);
                if (!tag.hasClass('stock')) {
                    $('#menu-package-details').data('menu-load',
                        '{{ url('restaurants/' . $restaurant->id) }}/reservation/package-details/' +
                        tag.data('id') + '/' + tag.data('date'));
                    var t = tag.find('.quantity-chairs input');
                    console.log(tag.data());
                ...
                if (place.id == placeId) {
                    ...
                    $.each(place.periods, function(k1, period) {
                        ...
                        if (period.type == 'table') {
                            checkTable = true;
                            content += '<div class="time-card ' + (tableCount <= 0 ? 'stock' : '') +
                                '" data-id="' + period.id + '">\
                										<div class="time-card-header text-center">' + period.from_string + ' @lang('dashboard.to') ' + period
                                .to_string + '</div>\
                										...
                										<p class="text-center remaining">' + (tableCount <= 0 ?
                                    '{{ trans('dashboard.out_of_stock') }}' :
                                    '{{ trans('dashboard.remaining') }} ' + tableCount) + '</p>\
                    ...
                    $.each(place.periods, function(k1, period) {
                        ...
                        if (period.type == 'chair') {
                            ...
                            chairContent += '<div class="time-card chair ' + (maxCount <= 0 ? 'stock' :
                                    '') + '" data-id="' + period.id + '">\
                										<div class="time-card-header text-center">' + period.from_string + ' @lang('dashboard.to') ' + period
                                .to_string + '</div>\
                										<div class="time-card-body text-center">\
                											<sp>' + period.chair_min + '</sp><i class="fas fa-user-alt"></i>\
                											<span><span class="price" data-price="' + period.price + '">' + (period.price * period
                                    .chair_min) + '</span> {{ $country->currency }}</span>\
                										</div>\
                										' + (maxCount <= 0 ? '<p class="text-center remaining">{{ trans('dashboard.out_of_stock') }}</p>' :
                                    '<div class="quantity-chairs" data-min="' + period.chair_min +
                                    '"  data-max="' + maxCount + '">\
                											<button type="button" class="btn plus">+</button>\
                											<input type="text"  data-type="chair" readonly name="qty" value="' + period.chair_min + '">\
                											<button type="button" class="btn sub">-</button>\
                										</div>') + '<p class="text-center remaining">' + (maxCount <= 0 ?
                                    '{{ trans('dashboard.out_of_stock') }}' :
                                    '{{ trans('dashboard.remaining') }} ' + maxCount) + '</p>\
                										<p class="text-center" style="font-size:10px !important;">(' + period.branch_name + ')</p>\
                									</div>';
                    ...
                    $.each(place.periods, function(k1, period) {
                        ...
                        if (period.type == 'package') {
                            checkPackage = true;
                            packageContent += '<div class="time-card package ' + (maxCount <= 0 ? 'stock' :
                                    '') + '"  data-id="' + period.id + '" data-date="' + period.date +
                                '" data-min="' + (period.chair_min > 0 ? period.chair_min : 1) + '" ' + (
                                    maxCount <= 0 ? '' : 'data-menu="menu-package-details"') + '>\
                											<div class="package-header">\
                												<img src="{{ asset('uploads/reservation_tables') }}/' + period.image + '" alt="">\
                											</div>\
                											<div class="time-card-header text-center">' + period.from_string + ' @lang('dashboard.to') ' +
                                period
                                .to_string + '</div>\
                										...
                										<p class="text-center remaining">' + (maxCount <= 0 ? '{{ trans('dashboard.out_of_stock') }}' :
                                    '{{ trans('dashboard.remaining') }} ' + maxCount + '') + '</p>\
                										<p class="text-center" style="font-size:10px !important;">(' + period.branch_name + ')</p>\
                										</div>';
                    ...
                    if (place.periods.length == 0) {
                        ...
                        $('#tables').html(
                            '<p class="text-center">{{ trans('messages.reservation_not_have_periods') }}</p>');
                    } else {
            ...
            $.ajax({
                ...
                success: function(json) {
                    ...
                    $.each(places, function(k, place) {
                        var image = place.image_path != null ? "{{ asset('') }}/" + place
                            .image_path : '{{ asset($restaurant->image_path) }}';
