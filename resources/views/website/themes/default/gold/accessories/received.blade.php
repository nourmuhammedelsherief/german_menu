                ...
                <div class="col-8 mb-1 text-left">
                    <p class="font-14 font-600  color-theme">
                        @if($order->payment_method == 'receipt_payment')
                            {{app()->getLocale() == 'ar' ? 'الدفع عند الأستلام' : 'Receipt Payment'}}
                        @elseif($order->payment_method == 'loyalty_point')
                            {{trans('messages.loyalty_point')}}
                        @else
                            {{app()->getLocale() == 'ar' ? 'الدفع الأونلاين' : 'Online Payment'}}
                        @endif
                    </p>
                </div>

                <div class="col-4 mb-1">
                    <p class="font-15 font-700 color-theme"> @lang('messages.receipt_method') </p>
                </div>
                <div class="col-8 mb-1 text-left">
                    <p class="font-14 font-600 color-theme">
                        @if($order->type == 'delivery')
                            {{app()->getLocale() == 'ar' ? 'ديليفري' : 'delivery' }}
                        @elseif($order->type == 'takeaway')
                            {{app()->getLocale() == 'ar' ? 'أستلام من الفرع' : 'TakeAway' }}
                        @elseif($order->type == 'previous')
                            {{app()->getLocale() == 'ar' ? 'طلب مسبق' : 'Previous Order' }}
                        @endif
                    </p>
                </div>
